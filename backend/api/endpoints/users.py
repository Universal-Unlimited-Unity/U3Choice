...
from services.users import get_friends_brief, update_user_profile, update_user_profile_photo, get_friends_brief
...
async def update_user_profile_endpoint(username: Annotated[str, Path()], update_data: Annotated[User_Update, Body()], user: Annotated[dict, Depends(verify_token)]):
    ...
    return {"token": await update_user_profile(update_data, user.get("username")), 
