...
async def send_verification_email_endpoint(background_tasks: BackgroundTasks, token: Annotated[dict, Depends(verify_token)]):
    ...
    if not user:
        logger.warning("User not found for ID %s", token.get("id"))
        raise HTTPException(status_code=404, detail="User not found")
    if user.status != "Active":
        logger.warning("User with ID %s is not active", token.get("id"))
    ...
    logger.info("Verification email sent successfully for user with ID %s", token.get("id"))
...
async def verify_email_code_endpoint(code: int, token: Annotated[dict, Depends(verify_token)]):
    ...
    logger.info("Email verified successfully for user with ID %s", token.get("id"))
...
async def forgot_password_endpoint(email: str, code: int, new_password: str):
    ...
    logger.info("Password updated successfully for user with ID %s", user.get("id"))
